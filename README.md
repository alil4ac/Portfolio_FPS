...
- 힘, 의지, 지능은 캐릭터의 스킬의 쿨다운을 감소하거나 강화하는 스탯.
  
- 물리 무기, 속성 무기, 파워 무기 3종의 무기 슬롯.
- 머리, 장갑, 갑옷, 다리, 악세서리 5종의 방어구 슬롯.
...
- 캐릭터의 패시브는 상 과 조각 두 종류가 있음.
- 상은 캐릭터의 능력에 효과를 추가하거나 새로운 액션 커맨드 제공.
- 조각은 기존의 행동이나 효과를 강화하거나 능력 재생, 회복등을 제공.

## B. 무기 개요
- 일반적인 무기는 등급에 따라 4개의 랜덤 특성을 가질 수 있음.
- 고유 등급 무기는 4개의 특성이 고정되어 있으며 고유한 특성을 추가로 가짐(총 5개 특성).
  
- 4개의 랜덤 특성 중 1,2 특성은 무기의 스펙에만 영향을 줌.
- 3,4 특성은 조건을 트리거로 버프, 디버프, 이펙트를 발동하는 특성.
  
- 무기는 탄약별로 종류가 다르며 일반 탄약, 특수 탄약, 중화기 탄약으로 분류됨.
- 일반 탄약 무기는 핸드건, 자동소총(돌격소총) 2개를 계획.
- 특수 탄약은 산탄총, 저격총, 차징형 무기 3개를 계획.
- 중화기는 머신건, 로켓발사기, 유탄 발사기 3개를 계획.

## C. 방어구 개요
- 방어구는 캐릭터의 스탯을 증감하는 주 역할.
- 방어구는 등급이 높을 수록 스탯의 총합이 높음.
- 각 방어구는 랜덤으로 스탯이 부여되지만 부위 별로 가중치를 적용.

## D. 적 유닛 개요
- 적 유닛의 등급은 일반, 정예, 미니 보스, 보스 4종 계획
...
### 1. 칼 묘기
- 전방에 부채꼴로 3개의 칼을 투척하여 각각 피해를 주고 칼 하나당 [소각](#소각)을 20부여.
- 칼날 투사체는 낙차가 존재.

### 2. 연쇄 번개
- 다음 근접 공격이 대상에게 [충격](#충격)을 부여하고 주변에 연쇄번개를 1회 발동.
- 캐릭터가 [증폭](#증폭)중에는 연쇄번개가 한번 더 발동.

### 3. 강타
- 다음 근접 공격이 대상과 그 반경 범위 내의 적에게 [억압](#억압)을 부여함.
- 해당 능력으로 적을 처치 시 45의 [보호막](#보호막)을 획득.

## 특수 능력

### 회피 도약
- 방향키로 캐릭터가 회피. 도중에는 피격 받지 않음.
- 회피 도약 이후 현재 들고 있는무기 즉시 재장전.
